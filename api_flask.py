...
def clean_up_files():
    # 删除 WORKING_DIR 目录下的所有 sdf 和 pdbqt 格式文件
    for file in WORKING_DIR.glob("*.sdf"):
        os.remove(file)
    for file in WORKING_DIR.glob("*.pdbqt"):
        os.remove(file)
    
    # 删除 UPLOAD_FOLDER 目录下的所有文件
    for file in UPLOAD_FOLDER.iterdir():
        os.remove(file)
    
    # 删除 DOWNLOAD_FOLDER 目录下的所有文件
    for file in DOWNLOAD_FOLDER.iterdir():
        os.remove(file)
    
    # 删除 gpf 文件夹及其所有文件
    gpf_folder = WORKING_DIR / "gpf"
    if gpf_folder.exists() and gpf_folder.is_dir():
        for item in gpf_folder.glob("*"):
            if item.is_dir():
                shutil.rmtree(item)
            else:
                os.remove(item)
    
    # 删除 WORKING_DIR 下的 pb 文件夹中的所有文件
    pb_folder = WORKING_DIR / "pb"
    if pb_folder.exists():
        for file in pb_folder.iterdir():
            os.remove(file)
    
    # 删除 WORKING_DIR 下的 dock/adgpu 文件夹中的所有文件
    adgpu_folder = WORKING_DIR / "dock/adgpu"
    if adgpu_folder.exists():
        for file in adgpu_folder.iterdir():
            os.remove(file)
    
    # 删除 WORKING_DIR 下的 dock/vina 文件夹中的所有文件
    vina_folder = WORKING_DIR / "dock/vina"
    if vina_folder.exists():
        for file in vina_folder.iterdir():
            os.remove(file)
...
class ConformationEvaluation(WorkflowStep):
    """构象评估步骤"""
    def __init__(self, working_dir: str, mode: str, mol_file: str, dock_mode: str, cond_file: str):
        ...
        # 自动根据 mol_file 的前缀推导 true_file
        mol_file_path = Path(mol_file)
        true_file_prefix = '_'.join(mol_file_path.stem.split('_')[:3])
        possible_true_file = Path(self.working_dir) / f"{true_file_prefix}.pdbqt"

        if not possible_true_file.exists():
            raise FileNotFoundError(f"未找到对应的 true_file: {possible_true_file}")

        self.true_file = possible_true_file
    ...
    def run(self) -> bool:
        ...
        # 调用 pb.py 进行评估
        pred_path = self.working_dir / pred_processed
        true_path = self.working_dir / true_processed
        cmd3 = [
        ...
        try:
            ...
            #将结果文件移动到 downloads 目录(追加写入)
            result_filename = "posebusters_results.csv"
            result_path = self.downloads_dir / result_filename

            # 读取新的结果文件
            new_df = pd.read_csv(self.output_csv)

            if result_path.exists():
                # 如果结果文件已存在，则追加写入，不包含表头
                new_df.to_csv(result_path, mode='a', header=False, index=False)
                logging.info(f"Appended results (no header) to existing file at {result_path}")
            else:
                # 否则写入并包含表头
                new_df.to_csv(result_path, index=False)
                logging.info(f"Created new results file at {result_path}")
...
def molecular_docking():
    ...
    if step.run():
        ...
        if dock_mode == "vina":
            # Vina 模式：只返回单一文件 【vina.pdbqt】
            base_name = Path(step.ligand_sdf).stem.split('_')[0]
            pattern = f"{base_name}_ligand_*_vina.pdbqt"
            vina_files = list(output_dir.glob(pattern))
            if vina_files:
                download_urls = [f"/api/download/molecular_docking/{f.name}" for f in vina_files]
            else:
                # 如果没有在downloads目录找到，也尝试在vina_output_dir中查找
                vina_output_dir = WORKING_DIR / "dock" / "vina"
                vina_files = list(vina_output_dir.glob(pattern))
                if vina_files:
                    # 将文件移动到downloads目录
                    for f in vina_files:
...
def conformation_evaluation():
    # 检查上传的文件
    required_files = ['pred_file', 'cond_file']
    ...
    if step.run():
        ...
        msg = "conformation evaluated successfully"
...
@app.route('/api/download_batch/<step_name>', methods=['POST'])
def download_batch(step_name: str):
    """批量下载指定步骤的所有结果文件"""
    ...
    if not os.path.exists(output_dir):
        try:
            ...
            return jsonify({"error": f"无法创建输出目录: {str(e)}"}), 500
